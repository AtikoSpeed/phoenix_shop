defmodule PhoenixShop.AccountsTest do
  ...
  describe "update_user_email/2" do
    ...
    test "does not update email if token expired", %{user: user, token: token} do
      {_, nil} = Repo.update_all(from(t in UserToken, where: t.user_id == ^user.id), set: [inserted_at: ~N[2020-01-01 00:00:00]])
  ...
  describe "get_user_by_session_token/1" do
    ...
    test "does not return user for expired token", %{user: user, token: token} do
      ...
      {_, nil} = Repo.update_all(from(t in UserToken, where: t.user_id == ^user.id), set: [inserted_at: dt, authenticated_at: dt])
